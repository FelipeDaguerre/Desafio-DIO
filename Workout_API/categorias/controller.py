...
@router.post(path='/', summary='Criar uma nova categoria',
...
async def post(db_session: DatabaseDependency, categoria_in: CategoriaIn = Body(...)) -> CategoriaOut:
    ...
    categoria_model = CategoriaModel(**categoria_out.model)
...
async def query(id: UUID4, db_session: DatabaseDependency) -> CategoriaOut:
    ...
    if not categoria:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f'Categoria no ID:{id} não encontrada.')
    return categoria